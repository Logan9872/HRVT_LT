...
# -------------------------------------数据读取和预处理----------------------------------------
# 遍历在文件夹中的文件并读取
dirs = os.listdir("E:/12.13乳酸阈数据处理/12.14室内跑数据/室内结果/2min合并数据/")
for all_files in dirs:
    file = "E:/12.13乳酸阈数据处理/12.14室内跑数据/室内结果/2min合并数据/" + all_files + ""
    ...
    y0 = CSV_data['SD1']
    y1 = CSV_data['SD2']
    y2 = CSV_data['DFA Alpha1']
    y5 = CSV_data['DFA Alpha2']
    ...
    sum_data_dist = {"Subject": subject, "LT_HR": la_HR, "LT_Speed": la_speed, "LT_Value": la_value}
    ...
    print(filename)
    plt.show()
    sum_data.to_csv("E:/12.13乳酸阈数据处理/12.14室内跑数据/室内结果/2min_HRVT/HRVT_data.csv", index=False)
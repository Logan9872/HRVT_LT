...
def remove_outliers_and_interpolate(data, window_size=40, z_threshold=3):
    if 'RRI' in data.columns:
        ...
        for i in range(len(rr_intervals)):
            ...
            # 判断异常值的上下限
            if rr_intervals[i] >= outlier_threshold1 or rr_intervals[i] <= outlier_threshold2:
                rr_intervals[i] = local_median
        ...
        #     interp = interpolate.interp1d(indices[not_nan], rr_intervals[not_nan], kind="zero", bounds_error=False, fill_value="extrapolate")
        #
...
# input_directory_path = "E:/12.13乳酸阈数据处理/12.14室内跑数据/merageDataMM/"
# output_directory_path = "E:/12.13乳酸阈数据处理/12.14室内跑数据/merageDataMM/"

input_directory_path = "E:/12.13乳酸阈数据处理/12.14户外跑数据/setDataMM/"
output_directory_path = "E:/12.13乳酸阈数据处理/12.14户外跑数据/setDataMM/"
